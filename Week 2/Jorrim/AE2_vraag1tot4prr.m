...
r_beta=(0:1:500);
for j=1:length(r_beta)
 rng(seed);
for i=1:Rep
        psi_1 = chi2rnd(1);
        psi_4 = chi2rnd(4-1);
        psi_11 = chi2rnd(11-1);
        LR4(i,j) = 0.5*(psi_4 + psi_1 - r_beta(j) + sqrt((psi_4 + psi_1 + r_beta(j))^2-4*r_beta(j)*psi_4));
        LR11(i,j) = 0.5*(psi_11 + psi_1 - r_beta(j) + sqrt((psi_11 + psi_1 + r_beta(j))^2-4*r_beta(j)*psi_11));
    end
end
...
%% VRAAG 1
    ...
    for alpha=1:length(a)            
        ...
        rejection_freq_LR(:,alpha) = mean(LR>=chi2inv(0.95,10))';
    end
...
%% PLOTS VRAAG 2 EN 4
figure
plot(r_beta,LR11_crit)
title('LR critical values for k=11')
ylim([0,25])
   
figure
plot(r_beta,LR4_crit)
title('LR critical values for k=4')
ylim([0,15])
